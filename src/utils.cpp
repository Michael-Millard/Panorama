...
bool convert_heic_to_jpg(const std::filesystem::path& heic_path,
    ...
    if (!std::filesystem::exists(heic_path) || !std::filesystem::is_regular_file(heic_path)) {
        if (error_message) *error_message = "Input file does not exist or is not a regular file";
        return false;
    }
    if (!is_heic_file(heic_path)) {
        if (error_message) *error_message = "Input file is not .heic/.heif";
        return false;
    }
    ...
    if (img.empty()) {
        if (error_message) *error_message = "Failed to read HEIC image (check OpenCV HEIF support)";
        return false;
    }
    ...
    if (!cv::imwrite(jpg_path.string(), img, params)) {
        if (error_message) *error_message = "Failed to write JPEG file";
        return false;
    }
    ...
    if (ec) {
        ...
        if (error_message) *error_message = std::string("Converted, but failed to remove original: ") + ec.message();
        if (out_jpg_path) *out_jpg_path = jpg_path;
        return true;
    }
...
std::size_t convert_heic_to_jpg_in_dir(const std::filesystem::path& dir,
    ...
    if (!std::filesystem::exists(dir) || !std::filesystem::is_directory(dir)) {
        if (report) *report = "Directory does not exist or is not a directory";
        return 0;
    }
    std::size_t converted = 0;
    std::ostringstream oss;
    auto handle_file = [&](const std::filesystem::path& p){
        if (!is_heic_file(p)) return;
        std::string err;
    ...
    if (recursive) {
        for (const auto& entry : std::filesystem::recursive_directory_iterator(dir)) {
            if (entry.is_regular_file()) handle_file(entry.path());
        }
    } else {
...
cv::Mat trim_black_bands_top_bottom(const cv::Mat& input,
    ...
    auto row_is_black = [&](int r) -> bool {
        ...
        for (int c = 0; c < cols; ++c) {
            if (ptr[c] <= max_black) {
                ++cnt;
                if (cnt >= row_black_limit) return true; // enough black to consider this row black
            }
        }
        return false;
    };
    ...
    int top = 0;
    while (top < rows && row_is_black(top)) ++top;

    int bottom = rows - 1;
    while (bottom >= 0 && row_is_black(bottom)) --bottom;
